...
namespace App\Http\Livewire\ReportSubgovernment;
...
class ReportByUserSubgovernment extends Component
{
    public $start_date;
    public $end_date;
    ...
    {
        return view('livewire.report-subgovernment.report-by-user-subgovernment');
    }
    ...
    {        
        if($this->management != '') {
            $this->datasubgoverment = Subgovernment::where('slug', auth()->user()->subgovernment_code)->firstOrFail();
    ...
    public function submit()
    {
        ...
        $this->subgoverment = Subgovernment::where('slug', auth()->user()->subgovernment_code)->firstOrFail();
