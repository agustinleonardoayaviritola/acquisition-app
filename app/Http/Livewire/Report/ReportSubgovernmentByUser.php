...
class ReportSubgovernmentByUser extends Component
{
    public $start_date;
    public $end_date;
    ...
    {
        return view('livewire.report.report-subgovernment-by-user');
    }
    public function mount()
    {
        $this->subgovernments = Subgovernment::all();
        $this->getDeliveryDetails();
        $this->getUserSubgovernment();
    }
    ...
    public function updatedsubgovernmentid()
    {
        $this->getUserSubgovernment();
    }
    public function getUserSubgovernment()
    {        
        /* if($this->management != '') { */
            if( $this->subgovernment_id != '') {
                $this->datasubgoverment = Subgovernment::where('id', $this->subgovernment_id)->firstOrFail();
                $this->users = User::where('subgovernment_code', $this->datasubgoverment->slug)->get();
            }
            else {
...
/*         } else {
    ...
    public function submit()
    {
        ...
        $this->datadeliveryusers = DeliveryBasket::where('user_id', $this->user_id)->whereBetween('delivery_baskets.created_at', [$this->start_date, $this->end_date])->get();
        //dd($this->datadeliveryusers);
